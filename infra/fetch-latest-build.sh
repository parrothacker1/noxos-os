...
LATEST_PREFIX=$(aws s3 ls "s3://${S3_BUCKET}/full/" | awk '{print $2}' | sort | tail -1)
if [ -z "$LATEST_PREFIX" ]; then
  echo "no builds found under s3://${S3_BUCKET}/full/" >&2
  exit 1
fi

mkdir -p "$DEST/download"
aws s3 sync "s3://${S3_BUCKET}/full/${LATEST_PREFIX}" "$DEST/download"
